...
**Progreso global:** Fase 0 ✅ · Fase 1 ✅ · Fase 2 ✅ · Fase 3 ✅ · Fase 4 ✅ · Fase 5 🟦 (código listo, falta activar con tu cuenta Stripe) · Fase 6 ✅ · Fase 7 ✅ (solo queda repaso responsive móvil) · Fase 8 ✅ (B1–B8 hechos; **email Resend ACTIVO en modo prueba**; textos legales borrador pendientes de revisión del negocio; Stripe opcional) · **DESPLEGADO en Vercel** (`saas-madajesadomicilio.vercel.app`) y revisado en móvil · **Fase 9 · Agente FASE A ✅** (agente OpenRouter + sección admin "Agente") · **FASE C ✅ v1** (pestaña "Análisis": KPIs, resultado/canal, temas, actividad). Fase 8 "importantes restantes" ✅. · **FASE 12 ✅** (2026-07-03: 8/10, todo salvo 12.8 que va en Fase 11·B y 12.10 ya desplegado). · **FASE 11 ✅ COMPLETA** (2026-07-03: Bloque A consentimiento, Bloque B disponibilidad real, Bloque C excepciones por fecha, + B6 backstop y B7 avisos; todo verificado y desplegado). **SIGUIENTE opciones:** Fase 9·FASE B (WhatsApp/teléfono, [`08_AGENTE.md`](08_AGENTE.md)); activar Stripe (Fase 5, requiere cuenta del usuario); verificar dominio email (Resend); pasada E2E conjunta formal R1–R8. Ver arriba el detalle.
...
- [x] **B7** Aviso a la masajista sin disponibilidad en Mi Calendario ("No recibirás reservas") + **vista admin**: badge "⚠️ Sin disponibilidad" en `GestionMasajistas` (flag `sin_disponibilidad` calculado en `loadAllMasajistas`).
- [x] **B6** Backstop en BD (migración `_28`): trigger `reservas_check_disponibilidad` impide asignar/ofrecer/confirmar a una masajista fuera de su horario (usa `masajista_en_horario`, refactorizado de `masajista_disponible`). Aplica a todos los actores (estricto). **Verificado**: ofrecer viernes → error; lunes 10:00 → OK.
...
- 2026-07-03 · **FASE 11 · FLECOS FINALES — B6 (backstop) + B7 (vista admin) → FASE 11 COMPLETA** · B6: migración `_28`, `masajista_en_horario` (horario sin solape) + trigger `reservas_check_disponibilidad` que impide asignar/ofrecer/confirmar fuera del horario (estricto, todos los actores; scope: entra en aceptada/ofrecida o cambian datos; NO en aceptada→completada para no romper histórico). **Verificado** (ofrecer viernes→error, lunes 10:00→OK). B7: badge "⚠️ Sin disponibilidad" en `GestionMasajistas` (flag `sin_disponibilidad` en `loadAllMasajistas`; disponibilidad tiene `disp_select_all`). · tsc 0 + build OK. **Fase 11 (A+B+C+flecos) cerrada.** Pendiente solo: pasada E2E conjunta formal R1–R8 (verificado por piezas).
